...
export function ResolutionCalculator({ locale }: { locale: Language }) {
  ...
  return <section className="resolution-workbench">
    <div className="resolution-controls">
      ...
      <div className="resolution-section resolution-preference"><div className="resolution-section-heading"><p className="kicker">03 / {t.preference}</p></div><label><select value={preference} onChange={(event) => setPreference(event.target.value as Preference)}><option value="1">{t.any}</option><option value="2">{t.even}</option><option value="4">{t.four}</option><option value="8">{t.eight}</option><option value="10-preferred">{t.ten}</option><option value="16">{t.sixteen}</option></select></label></div>
      <div className="resolution-section resolution-preference resolution-limit"><div className="resolution-section-heading"><p className="kicker">04 / {t.limit}</p></div><label><select value={growthLimit} onChange={(event) => setGrowthLimit(event.target.value as GrowthLimit)}><option value="none">{t.limitNone}</option><option value="source">{t.limitSource}</option><option value="percentage">{t.limitPercentage}</option></select></label>{growthLimit === "percentage" ? <label className="resolution-percentage"><span>{t.percentage}</span><input type="number" min="0" step="1" value={maxGrowth} onChange={(event) => setMaxGrowth(event.target.value)} /></label> : null}</div>
